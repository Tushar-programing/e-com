    ...
    <div className='h-36 border'>
        <Link to={`/post/${slug}`}>
            <div className='float-left w-48 h-auto'>
              {image[0]? <img className='mt-5 ml-14' style={{ height: '100px', }} src={authService.getFilePreview(image[0])}></img> : null}
            </div>
        ...
        <p className='text-xl float-left font-medium ml-20 mt-10'>Total ₹ {price * quantity}</p>
    </div> 
  )
}
